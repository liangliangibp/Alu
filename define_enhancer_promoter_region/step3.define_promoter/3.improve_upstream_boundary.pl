...
while(my $line=<PAB>){
	...
	if($sub[5] eq "+"){
		if(exists $promoter_boundary{$raw_promoter_id}{5} and $sub[8] < $promoter_boundary{$raw_promoter_id}{5}){
			$promoter_boundary{$raw_promoter_id}{5}=$sub[8];
		}
		elsif(!exists $promoter_boundary{$raw_promoter_id}{5}){
			$promoter_boundary{$raw_promoter_id}{5}=$sub[8];
		}
		else{
			next;
		...
		if($sub[9] > $promoter_boundary{$raw_promoter_id}{5}){
			$promoter_boundary{$raw_promoter_id}{5}=$sub[9];
		}
	}
}
...
foreach my $raw_promoter_id (sort keys %promoter_boundary){
	...
	if($sub[-1] eq "+"){
		if(!exists $promoter_boundary{$raw_promoter_id}{5}){#no peaks in the upstream
			next;
		}
		if($promoter_boundary{$raw_promoter_id}{5} < $sub[1]){
		...
		if($promoter_boundary{$raw_promoter_id}{5} > $sub[2]){
